...
public:
    ...
    bool dijkstra_Test(int startID, int endID, QList<int> &path,double &cost);//主要测试路径规划算法
    double WeigthCal(QList<int>path,int angle,TopoEdge*p,float v,float w);
    ...
    void CommandProduce(int current_node,QList<int> path, QList<AGVCommand>&Command,int angel,QString agv_name);//将AGV的path 生成一系列的Command;
    void CommandProcess(int startnode,int end,int angel);//测试函数
    //void PathProduce(AGV*agv,current_node,target_node,QList<int>&path);


    QMap<int,AGVCommand>Command;
    QMap<QString, QList<AGVCommand>>agv_command;
    QMap<int,int>lockpoint;//独占区   point--agv
